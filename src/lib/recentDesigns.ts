...
export function readRecentDesigns(): RecentDesignEntry[] {
  try {
    ...
    return parsed.filter(isEntry).sort((a, b) => b.lastOpenedAt - a.lastOpenedAt);
  } catch {
    return [];
  }
}

export function addRecentDesign(entry: { id: string; url?: string; source: RecentDesignSource }): void {
  ...
  try {
    ...
    const next: RecentDesignEntry[] = [{ id: entry.id, url, lastOpenedAt: now, source: entry.source }, ...without].slice(
      0,
      MAX_RECENT
    );
